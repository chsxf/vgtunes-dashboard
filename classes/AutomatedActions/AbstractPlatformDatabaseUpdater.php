...
abstract class AbstractPlatformDatabaseUpdater extends AbstractSequentialAutomatedAction
{
    ...
    public function setUp(DataValidator $validator): void
    {
        ...
        $sql = "SELECT DISTINCT `album_id`
                    ...
                    WHERE `album_id` NOT IN (SELECT DISTINCT `album_id` FROM `album_instances` WHERE `platform` = ?)";
        $values = [$this->getPlatform()->value];
        if ($firstId > 0) {
            $sql .= " AND `album_id` >= ?";
            $values[] = $firstId;
        }
        ...
        if (($ids = $dbConn->getColumn($sql, $values)) === false) {
            throw new Exception('Unable to fetch album Ids with missing link');
        }
    ...
    public function proceedWithNextStep(): AutomatedActionStepData
    {
        ...
        try {
            ...
            $platformHelper = PlatformHelperFactory::get($this->getPlatform(), $this->coreServiceProvider);
            ...
            if ($exactMatch === null) {
                ...
                if ($dbConn->exec($sql, $currentAlbumId, $this->getPlatform()->value, $exactMatch['platform_id']) === false) {
